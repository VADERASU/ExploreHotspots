class KDERenderer {
  constructor(leafletMap){
    ...
    this.catMapSize = 128;
    this.catMap = this.createTexture(1,[1,this.catMapSize],new Float32Array(this.catMapSize));
    this.setCatMap();
    ...
    this.uniforms = {
      ...
      catMap: {type:'t', value: this.catMap},
  ...
  setCatMap(data, catIndex){
    const values = this.catMap.image.data;

    if(data){
      for(let i=0; i<data.length; i++)
        values[i] = data[i][0][catIndex][0];
    } else {
      for(let i=0; i<this.catMapSize; i++)
        values[i] = i;
    }

    this.catMap.needsUpdate = true;
  }
...
void main() {
    ...
    vec4 segmentation = floor( texture2D(texSegmentation, vUV)*256.0 );
    ...
    if(id<255.0 && (${this.consts.branchID}.0<0.0 || id==${this.consts.branchID}.0)){
      float catId = texture2D(catMap, vec2(0,id/${this.catMapSize}.0)).a;
      color = texture2D( colorMap, vec2(
          0,
          mod(catId,nColors)/15.0
        )).rgb;
    }
  ...
  render(idIdx, bgIdx, opacity, nContours, contourWidth, branchID){
