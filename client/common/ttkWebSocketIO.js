...
class vtkDataSet {
    constructor(className){
        ...
        this.points = {
            coordinates: new vtkArray("coordinates", 3,0,CONSTS.VTK_FLOAT,[])
        };

        this.cells = {
            offsetsArray: new vtkArray("offsetsArray", 1,1,CONSTS.VTK_INT,[0]),
            connectivityArray: new vtkArray("connectivityArray", 1,0,CONSTS.VTK_INT,[])
        };
...
class vtkArray {
    constructor(name, nComponents, nTuples, dataType, data){
        this.name = name;
    ...
    static async createFromBinaryData( header, binaryData ){
        ...
        array.name = header.name;
        array.dataType = header.dataType;
        array.nTuples = header.nTuples;
        array.nComponents = header.nComponents;
...
class ttkWebSocketIO extends Base {
    ...
    sendVTKDataSet( object ){
        ...
        this.sendString( JSON.stringify({"vtkDataSet":object.toJSON()}) );
    }
}
